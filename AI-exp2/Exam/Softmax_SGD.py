...
def stocSoftmaxRegression(data, label):
    ...
    theta = zeros((m, 3))  # m features, 3 tpyes + 1
    alpha = 0.001
    maxCycle = 5000
    episilon = 1e-7
    preLikelihood = 0.0
...
def main():
    ...
    print("Softmax随机梯度下降对Exam训练数据集二分类的准确率:")
    print(getCorrectRate(predictlabel2, mat(label).transpose()))
    plotBestFit(fig, trainData, label, theta2, "stocSoftmax, ToTrainDataSet", 121)

    # 划分测试数据集
    print("to TestDataSet:")
    predictlabel1 = getTestPredict(theta2, testdata)
    print("Softmax随机梯度下降对Exam测试数据集二分类的准确率:")
